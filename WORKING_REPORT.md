...
## 2026-05-24 07:52 CST - Readiness-run persisted summary control_surface

### ??????
- ? `readiness-run` ? persisted `.asteria/readiness_runs/<id>/summary.json` ??? `control_surface` ????? CLI/result JSON?docs fixture??? summary schema ??????

### ????????
- `ReadinessRunCommand._build_summary()` ?? `control_surface`?`schemas/readiness_run.schema.json` ????????????? blocked/dry-run/completed summary schema?

### ????????
- `src/asteria_runtime/commands/readiness_run_command.py`
- `schemas/readiness_run.schema.json`
- `tests/unit/test_readiness_run_command.py`
- `tests/unit/test_documentation_contracts.py`
- `WORKING_REPORT.md`

### ??????
- `readiness_run_command.py`?????? `_readiness_run_control_surface()` ?? result JSON ? summary JSON?????????????
- `readiness_run.schema.json`?? `control_surface` ?? readiness_run persisted schema ? required/properties?????????????????????
- `test_readiness_run_command.py`?? blocked?dry_run?completed ???????? summary ??? control_surface + readiness_run schema??????????
- `test_documentation_contracts.py`??? schema contract ????? readiness_run schema ?????? control_surface ???
- `WORKING_REPORT.md`????????????????

### ????
- `python -m pytest tests/unit/test_readiness_run_command.py tests/unit/test_documentation_contracts.py tests/unit/test_control_surface_commands.py tests/unit/test_schema_validator.py -q`?49 passed?
- `ruff check src/asteria_runtime/commands/readiness_run_command.py tests/unit/test_readiness_run_command.py tests/unit/test_documentation_contracts.py`?All checks passed?

### ?????
- ???? `.asteria/readiness_runs/*/summary.json` ??? `control_surface`???? schema ??????????????????????????????? schema_version ?? 0.1.0????? schema_version ???????

### ???????????????
- ?? readiness_run summary ????/????????? `control_surface` schema ?????????/??????

### ??????????
- ?? persisted readiness_run summary ?? `control_surface` ????????????? readiness_run summary ?? schema migration ????????

## 2026-05-24 07:36 CST - Readiness-run control_surface fixture contract

### ??????
- ? `readiness-run --json` ?????? fixture ? documentation contract???????? `maintainer_readiness_execution` ???? runtime ????????????????

### ????????
- ?? `docs/en/examples/readiness_run_control_surface.json`???????????? fixture ? command/audience/stable_fields ? runtime payload ?????
...
### ??????
- `readiness_run_control_surface.json`?? Studio/CI/?????????????????? `readiness-run --json` ??????
- `test_documentation_contracts.py`?? readiness-run ???? runtime stable_fields ????? documented audience ????? fixture ?????????
- `docs/zh/????.md`?? readiness-run fixture ?????????????? source-of-truth ????????
- `WORKING_REPORT.md`????????????????

### ????
- `python -m pytest tests/unit/test_documentation_contracts.py tests/unit/test_readiness_run_command.py tests/unit/test_control_surface_commands.py -q`?44 passed?
- `ruff check tests/unit/test_documentation_contracts.py tests/unit/test_readiness_run_command.py tests/unit/test_control_surface_commands.py`?All checks passed?

### ?????
- `readiness-run` ??? persisted summary JSON ???? `control_surface`?????? CLI/result JSON ? docs fixture????? schema ?????
- `docs/zh/????.md` ????????????????? Windows/PowerShell ????????????????? UTF-8 ???

### ???????????????
- ????? persisted `.asteria/readiness_runs/<id>/summary.json` ?????? `control_surface`????????? `schemas/readiness_run.schema.json` ???/?????

### ??????????
- ?? `readiness_run_control_surface.json` ?????? Studio/CI ????????????? readiness-run summary ???????? control_surface?

## 2026-05-24 07:21 CST - Readiness-run execution control_surface

### ??????
- ? `readiness-run --json` ???????? `control_surface` ????? `readiness` dry-run readiness ???/?? readiness execution ????????????????

### ????????
- `ReadinessRunResult.to_dict()` ???? `schema_version` ? `control_surface`?audience ? `maintainer_readiness_execution`?stable_fields ?? readiness_run_id/status/summary_path/run_id/next_actions?
...
### ??????
- `readiness_run_command.py`?? `asteria readiness-run --json` ? version/package-check/status/doctor/gate/readiness ???????????? CI?Studio ???????????????? release readiness ? readiness execution?
- `test_readiness_run_command.py`??? blocked?dry_run?completed ?????????????? control_surface schema?audience ? stable_fields ?????
- `test_control_surface_commands.py`?? `ReadinessRunCommand` ???? control_surface ????????????? readiness-run ???
- `docs/zh/????.md`????????????? `readiness-run` ?? `maintainer_readiness_execution`???????? `readiness` readiness ????????
- `WORKING_REPORT.md`?????????????????

### ????
- `python -m pytest tests/unit/test_readiness_run_command.py tests/unit/test_control_surface_commands.py tests/unit/test_documentation_contracts.py -q`?44 passed?
- `ruff check src/asteria_runtime/commands/readiness_run_command.py tests/unit/test_readiness_run_command.py tests/unit/test_control_surface_commands.py`?All checks passed?

### ?????
- `readiness-run` ?????? example fixture?????????? control_surface ???????????? readiness-run payload ??? docs/en/examples?
- `readiness-run` ??? summary JSON ?? readiness_run schema???? control_surface?????? CLI/result JSON????? persisted schema ?????

### ???????????????
- ? `readiness-run --json` ???? example fixture ? documentation contract????????? persisted summary ???? `control_surface` ??? schema?

### ??????????
- ?? `maintainer_readiness_execution` audience ????? `maintainer_release_readiness` ??????????? `readiness-run` summary ????????? control_surface ???


## 2026-05-24 07:06 CST - Release readiness control_surface

### ??????
- ? `readiness --json` dry-run ???????? control_surface ???????? fixture???????????? `doctor -> gate -> readiness` ????????

### ????????
- `ReadinessCommandResult.to_dict()` ???? `control_surface`??? `readiness_control_surface.json` ??????????????? `readiness --json` ? audience ? stable_fields?
...
### ???????
- `readiness_command.py`?? `readiness --json` ?? `maintainer_release_readiness` control_surface??????? root/status/ok/mode/gate_status/readiness_run/next_actions?
- `readiness_control_surface.json`??? dry-run ??????? payload?? Studio/CI/?????
- `test_control_surface_commands.py`??? ReadinessCommand ??????????????????? dry-run blocked/ready ???
- `test_documentation_contracts.py`?? readiness ?????? schema ??? runtime stable_fields ?????
- `docs/zh/????.md`??? `readiness --json` ?? control_surface??? readiness ?????????
- `WORKING_REPORT.md`??????????????????
...
- `ruff check src/asteria_runtime/commands/readiness_command.py tests/unit/test_control_surface_commands.py tests/unit/test_documentation_contracts.py`?All checks passed?

### ?????
- `readiness_control_surface.json` ????? gate_status/readiness_run ???????????? readiness contract ? stable_fields ?????????? gate_status contract ???

### ???????????????
- ? `readiness_control_surface.json` ???? `gate_status.control_surface` ? `readiness_run` summary ??????????? `readiness-run --dry-run` ??? control_surface?

### ??????????
- ?? `maintainer_release_readiness` audience ????????????? readiness stable_fields ?????? Studio ??????
...
### ???????????????
- ? `readiness --json` dry-run ???? control_surface ?????? fixture????? `doctor -> gate -> readiness -> evidence-bundle` ?????????????????
...
- `docs/en/examples/gate_status_control_surface.json`??? `gate-status --json` release readiness ???????? release/readiness gate ?????????
...
### Suggested next medium-granularity target
- Add a compact CLI smoke/help contract for `gate`, `readiness`, and `acceptance` so maintainer-facing commands remain clearly separate from the default user workflow.
...
### Iteration goal
- Keep maintainer-facing validation commands (`gate`, `readiness`, `acceptance`, `acceptance-gate`) clearly outside the ordinary user completion workflow in CLI help and quick-start docs.
...
### Modified files
- `src/asteria_runtime/cli.py`: adds the maintainer command help note and wires it into `gate`, `readiness`, `acceptance`, and `acceptance-gate`.
...
### Reasons
- Recent iterations clarified the default workflow and `accept`/`acceptance`; this closes the adjacent UX gap for `gate` and `readiness`, which are important but should not become ordinary user mental-model requirements.
...
### Suggested review focus for tomorrow
- Review whether maintainer/CI language should also appear in the Chinese command reference around the `gate` and `readiness` sections.
