...
当前 ACTIVE_SLICE：G17（图片附件·1.2.96/1.2.98 **已收官**）。**2026-07-17 用户拍板后开的新队列**：G8 = **两者都要**（档位默认 + 可覆盖具体模型·分两刀）、G6 刀二 = **编辑框→喂模型重出**（编辑手感·模型语义·ADR-0016 相容）。进度：**1.2.103 warm worker 接通 + 权限档按数据穿透**（做 G8 追接线时撞见的真 bug：暖路从出生起不可达）→ **1.2.104 G8-a 档位选择器已落**（骑同一条通道·冷/暖两路真实用户路径均有探针实证）→ 下一刀 G8-b 具体模型 → G6 刀二。同批已收官：自审 8/8（1.2.97/1.2.99/1.2.100）、索引虚构纠正（1.2.101）、§16.1「近期第一批」结清（1.2.102）。已判**不做**并留痕：G18（等证据·8ae2b52）、`model_call_logger` O(N²)（量出 0.02%·1.2.101）、`package-check` 真构建（真闸已在 release.yml·1.2.102）。

**本段是索引，不是记账。细节一律去真源查，勿在此复述**（复述会漂，已犯过：见 1.2.95）：
- 执行计划 + 逐刀 changelog（**唯一真源**）：`docs/zh/研发总计划.md` §16 / §16.1 / changelog 1.2.30–1.2.104
...
- G8-b 具体 provider/model 覆盖——**G8-a 档位选择器已落**（1.2.104）。剩下的真缺口在下一层：
  `policy["model_routing"]` **零生产消费者**，且与 `_model_routing_overrides:116-122` 的设计注释
  （"别在 run 创建时把路由钉死"）冲突 ⇒ 要么推翻它并记 ADR，要么设计成窄覆盖。**真设计活，非接线**。
  机制与两条可选路径见路线 §G8。
...
执行顺序：Part B 前端拉齐剩余（**G8 已于 2026-07-17 拍板=两者都要**·G8-a 已落 1.2.104·下一刀 G8-b；G6 刀二形态已拍板=编辑框喂模型重出·其余按证据）→ P0 沙箱（唯一剩余 P0·按内部发动机定位已降级）→ 利基 Beta
