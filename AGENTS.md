...
当前 ACTIVE_SLICE：G17 图片附件（1.2.96/1.2.98）与自审清理批（1.2.97/1.2.99）均已收官。下一刀待定：G18 成本定价表 / 补 Efficiency+Altitude 两自审角度。

**本段是索引，不是记账。细节一律去真源查，勿在此复述**（复述会漂，已犯过：见 1.2.95）：
- 执行计划 + 逐刀 changelog（**唯一真源**）：`docs/zh/研发总计划.md` §16 / §16.1 / changelog 1.2.30–1.2.99
...
P2：G15 回放导出（1.2.90）、G14 记忆只读（1.2.91）、**G17 图片附件全案**（CLI 1.2.96 + Studio 粘贴 1.2.98）、自审清理批（1.2.97 修 6 条 + 1.2.99 清 8 条）。主区渲染/叙事序/即时感/
...
**仍开着**（每条都注明卡在哪，别当"没做完"）：
- G6 刀二 编辑式改计划——**卡在语义分叉待 DecisionPoint**：UI 上的"第 N 步"多半是 model_todos.json
  （模型独占面，编辑=空动作），有执行效力的是 task_plan.json。不是后端做不了，是不知该编哪份。
