...
当前 ACTIVE_SLICE：G17（图片附件·1.2.96/1.2.98 **已收官**）。**2026-07-17 用户拍板后开的新队列**：G8 = **两者都要**（档位默认 + 可覆盖具体模型·分两刀）、G6 刀二 = **编辑框→喂模型重出**（编辑手感·模型语义·ADR-0016 相容）。进度：**1.2.103 warm worker 接通 + 权限档按数据穿透**（做 G8 追接线时撞见的真 bug：暖路从出生起不可达）→ **1.2.104 G8-a 档位选择器**（冷/暖两路真实用户路径均有探针实证）→ **1.2.105/1.2.106 G8-b 两刀**（运行时通道+CLI·Studio UI+BFF·**开工调研推翻了路线图为 G8-b 写的两条路**·见 1.2.105）⇒ **G8 全案收官**（冷/暖两路真实用户路径均有探针实证）→ **1.2.107 G6 刀二**（编辑框→喂模型重出·零后端·不写任何计划文件 ⇒ UI 不说谎由构造保证）⇒ **用户拍板的队列已全部跑完**。同批已收官：自审 8/8（1.2.97/1.2.99/1.2.100）、索引虚构纠正（1.2.101）、§16.1「近期第一批」结清（1.2.102）。**2026-07-18 用户授权「按你建议进行」落地 S89 长期记忆补活水**：remember/recall_memory 模型通道 + 索引召回（替换最后8条尾巴）+ goal 换槽归档——真源见 1.2.122 / ADR-0031 / brief S89，此处不复述。**同日 S90 对话/上下文管理补活水**（用户「按你建议进行完善修复」）：循环历史微压缩 + per-model 真窗口/真 usage 校准 + 压缩真缩（重审计残余债第三位「压缩不缩活提示」已闭）——真源见 1.2.123 / ADR-0032 / brief S90。已判**不做**并留痕：G18（等证据·8ae2b52）、`model_call_logger` O(N²)（量出 0.02%·1.2.101）、`package-check` 真构建（真闸已在 release.yml·1.2.102）。
