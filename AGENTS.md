...
当前 ACTIVE_SLICE：G17（图片附件·1.2.96/1.2.98 **已收官**）——**其后自驱清单已跑空（1.2.102），下一刀待用户定**，故标记停在最后一刀而不新编一项（不给空槽填发明·子型 f）。同批已收官：自审 8/8 角度（1.2.97/1.2.99/1.2.100）、索引虚构纠正（1.2.101）、§16.1「近期第一批」结清（1.2.102）。已判**不做**并留痕：G18（等证据·8ae2b52）、`model_call_logger` O(N²)（量出 0.02%·1.2.101）、`package-check` 真构建（真闸已在 release.yml·1.2.102）。**剩余全部卡在用户**（拍板 / 目视 / 真 friction 证据 / OS 沙箱 P0）——见下「仍开着」。

**本段是索引，不是记账。细节一律去真源查，勿在此复述**（复述会漂，已犯过：见 1.2.95）：
- 执行计划 + 逐刀 changelog（**唯一真源**）：`docs/zh/研发总计划.md` §16 / §16.1 / changelog 1.2.30–1.2.102
