"""Maintainer gray check for Phase 5 swarm entry (S21)."""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run Phase 5 swarm maintainer gray checks.")
    parser.add_argument("--root", type=Path, default=Path("."), help="Repository root")
    parser.add_argument("--skip-studio", action="store_true", help="Skip Studio smokes")
    args = parser.parse_args()

    root = args.root.resolve()
    gate = json.loads((root / "benchmarks" / "phase5_swarm_gate.json").read_text(encoding="utf-8"))
