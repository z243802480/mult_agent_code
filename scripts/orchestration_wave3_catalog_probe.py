#!/usr/bin/env python3
"""Run Wave 3 spawn_parallel_workers catalog gray probe (S64)."""
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Wave 3 orchestration catalog gray probe")
    parser.add_argument("--root", type=Path, default=ROOT, help="Repository root")
    parser.add_argument(
        "--decision-id",
        default="decision-orchestration-parallel-0002",
        help="DecisionPoint id under .asteria/decisions/",
    )
    parser.add_argument(
        "--option",
        default="wave3_catalog_gray",
        help="Option to resolve (default: wave3_catalog_gray)",
    )
    parser.add_argument(
        "--write-decision",
        action="store_true",
        help="Persist Wave 3 DecisionPoint when readiness allows",
    )
    ...
    readiness = evaluate_wave3_catalog_readiness(root=root, policy=policy)
    ...
    if args.write_decision and readiness.ready_for_decision_point:
        decision_written = persist_orchestration_parallel_decision_point(
            ...
            decision_point=build_wave3_catalog_decision_point(
                run_id="orchestration-wave3-catalog-pulse",
                readiness=readiness,
            ),
        )

    try:
        result = run_orchestration_wave3_catalog_probe(
            repo_root=root,
            validator=validator,
