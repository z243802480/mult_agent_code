#!/usr/bin/env python3
"""Run Wave 4 orchestration workflows gray probe (S64-W4)."""
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Wave 4 orchestration workflows gray probe")
    parser.add_argument("--root", type=Path, default=ROOT, help="Repository root")
    parser.add_argument(
        "--decision-id",
        default="decision-orchestration-parallel-0003",
        help="DecisionPoint id under .asteria/decisions/",
    )
    parser.add_argument(
        "--option",
        default="wave4_workflows_gray",
        help="Option to resolve (default: wave4_workflows_gray)",
    )
    parser.add_argument(
        "--write-decision",
        action="store_true",
        help="Persist Wave 4 DecisionPoint when readiness allows",
    )
    ...
    readiness = evaluate_wave4_workflows_readiness(root=root, policy=policy)
    ...
    if args.write_decision and readiness.ready_for_decision_point:
        decision_written = persist_orchestration_parallel_decision_point(
            ...
            decision_point=build_wave4_workflows_decision_point(
                run_id="orchestration-wave4-workflows-pulse",
                readiness=readiness,
            ),
        )

    try:
        result = run_orchestration_wave4_workflows_probe(
            repo_root=root,
            validator=validator,
