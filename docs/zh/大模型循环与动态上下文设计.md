...
Asteria 不是只有一条 `run` 链路。更准确的理解是：用户侧只有 Goal / Plan / Ask 三种入口心智，runtime 内部有多条智能体链路为它们服务。普通用户不需要看到这些链路名，但工程实现和文档必须保留它们，否则后续会把调试命令、运维 gate 和用户产品路径混在一起。
...
| 多智能体链路 | `MultiAgentStrategyAdvisor`, `AgentRunGraphBuilder`, `worker_tree.py` | 对复杂任务拆分子 worker，记录父子关系、候选产物和冲突 | 默认隐藏；只输出整合后的进展和结果 |
| Context / Recovery 链路 | `compact_command.py`, `handoff_command.py`, `sessions_command.py`, `ContextSnapshot` | 跨上下文压缩、恢复、交接、长期任务连续性 | 长任务状态可见；恢复细节默认隐藏 |
| Promotion / Merge 链路 | `MergeGate`, `promotions_command.py`, `candidate_promotion_queue.py` | 处理候选工作区、合并门禁、人工批准或拒绝 | 普通用户不默认可见 |
| Acceptance / Gate / Gray 链路 | `acceptance_command.py`, `gate_command.py`, `gray_command.py`, `gray_run_command.py` | 维护者/CI 用来验证 runtime 能力、真实模型灰度和发布健康度 | 维护者视图，不是普通用户主线 |
...
- 并发不是默认越多越好，必须受预算、风险、上下文隔离、冲突概率和 provider readiness 控制。
...
flowchart TD
    ...
    Evidence --> Ops["Acceptance / Gate / Gray / Debug"]
```
...
- 验收链路：已有 gate status、small real task gray、review fallback。
...
1. 动态上下文选择还不够精确。
   - 需要明确每类请求应该挂载哪些上下文。
   - 需要防止 Chat 默认携带过多长期状态，导致回答被旧目标污染。
...
5. 长任务记忆还需要从实现能力变成产品协议。
   - 需要统一文件名、章节结构、更新时机、冲突处理和恢复策略。
   - 这个文件应成为长任务的永久记忆，而不是临时日志。

6. 真实任务灰度覆盖还不足。
   - 目前 small real task gray 已可继续推进，但任务复杂度仍偏低。
...
- Chat 默认只使用当前对话和轻量项目上下文。
- 仅当用户询问进展、计划、状态、下一步时挂载长期任务记忆。
- 普通回答不暴露 run_id、evidence、model_route、provider。

### P1：长任务记忆产品化

- 固定 `.asteria/memory/active_goal.md` 或等价长期状态文件结构。
- `/run`、`/accept`、修复、暂停、恢复时更新该文件。
- `status` 默认读取用户级长期状态。
- `status --debug` 才展示运行时细节。

### P1：工具 / MCP / Skills 调用策略
...
- 在 CLI/runtime loop 稳定后，再考虑 Studio / Dashboard。
- Dashboard 应展示用户任务状态、产物、验收结果、下一步，而不是优先展示底层 trace。
...
1. 修正 Chat 的上下文策略：默认不读取长期记忆，只在状态/计划/下一步类问题中挂载。
2. 把长期任务 Markdown 记忆正式定义为长任务状态协议。
3. 增加 Context Envelope 测试，确保 backend-only 字段不会泄漏到普通用户回答。
4. 用一个小真实任务灰度验证：让 Asteria 解释当前项目状态，并只返回用户级下一步。
5. 再推进工具 / MCP / Skills 调用策略表，把调用时机从“模型临场猜”变成“策略 + 模型判断 + 验证”。
