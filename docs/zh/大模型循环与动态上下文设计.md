...
> 2026-06-08 角色上下文投影：完整 `PromptEnvelope`、能力目录和 raw evidence 继续由 Runtime 持久化并供 Inspector 查证，但不得默认复制到每个模型调用。GoalSpec 只接收目标理解所需的项目摘要、相关历史/研究、预算与 route plan；Coder 接收 task contract、scoped files、最近 observation 和可用工具；Review 接收独立验证与结果证据。上下文选择按角色职责收敛，不按“可能有用”无限累加。
...
flowchart TD
    ...
    RP --> WK["Worker + Tools（+ 受控子 Agent）"]
    WK --> EV["证据与产物"]
    EV --> NX["模型在同一循环内判断下一步"]
    NX -->|repair / replan / ask（模型选择）| WK
    NX -->|完成| AC["Accept（Review 为显式可选动作）"]
...
| 修复与重规划链路 | `debug_command.py`（薄适配器 → `ExecuteCommand`）, `replan_command.py`, `resume_command.py` | 失败/缺证据/阻塞/计划失效时，由模型在单循环内选择 repair/replan/ask，或经显式 debug/replan/resume 命令触发；无独立 DebugAgent、无自动 Gate→Repair 路由 | 只在需要用户决策时打断 |
...
> **架构中心（ADR-0015）**：上表是链路的**产品语义边界**，不是 N 个并列的运行时引擎。运行时以单个 Session Agent Loop（`CoderAgent`）为中心；其余要么是显式命令入口（Review/Accept/debug/replan/resume），要么是可选支撑能力（Research/Brainstorm 默认不进入写入执行），要么是恢复适配器（`debug_command` 是 `ExecuteCommand` 的薄包装）。`DebugAgent` 目前是 deferred placeholder，**不在默认运行时路径**中（`src/asteria_runtime/agents/debug_agent.py`）。
...
flowchart TD
    ...
    Plan --> Exec["Coder / Worker / Tools（+ 子 Agent）"]
    Exec --> Evidence["Evidence / Validation"]
    Evidence --> Next["模型判断下一步（同一循环内）"]
    Next -->|repair / replan / resume / ask| Exec
    Next --> Review["Review / Accept（显式）"]
...
sequenceDiagram
    ...
    participant Worker as Worker / Tools（+ 子 Agent）
    ...
    Runtime->>Worker: observation 回灌；模型在同一循环内选择 repair/replan/ask 或继续
    Runtime->>Gate: 显式 Review / Merge Gate（动作边界，不自动路由修复）
...
- `BudgetController.pressure()` 将 `context_window` 作为独立压力维度；`/run` 在 near-limit 时**写入可恢复快照并给出压缩建议（`compact_boundary`），当前只测量+建议、不自动缩减发送给模型的 live 上下文**，在 hard-stop 时创建预算 DecisionPoint。（`policies` 的 `phase_boundary_compaction`/`handoff_compaction` 为预留旗标，运行时当前未消费。）
