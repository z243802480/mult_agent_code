...
发布准备验证阶段的默认策略是 GLM coding route 做 strong/coordinator，MiniMax 做 medium/worker。MiniMax 的规划能力不作为主路线假设，优先用于编码、修复、诊断等明确边界内的执行任务；GLM coding route 优先承接 goal/spec、planning、review、research 和后续 coordinate。本项目当前可先使用 `glm-4.7` 消耗免费 token 额度，额度用完、质量不足或进入最终 release gate 时再切到 `glm-5.1` 复验。未来接入 GPT-5.5 时，应先作为 `strong` route 替换 coordinator，而不是改动 worker runtime。
...
当前发布准备验证默认先收紧最坏路径：真实 acceptance 默认每个场景 1 次 run attempt、模型层 1 次 retry、单场景 600 秒；子进程会把 provider call 默认压到 90 秒以内，review/recovery 默认压到 90 秒以内。用户显式配置的 `AGENT_MODEL_<TIER>_TIMEOUT_SECONDS` 和 `AGENT_MODEL_<TIER>_MAX_RETRIES` 优先级高于发布准备验证默认值。
...
- 非流式仍保留为兼容 fallback，但 readiness/release gate 应优先要求真实 strong/medium route 具备流式证据，否则只能进入受限发布准备验证。
