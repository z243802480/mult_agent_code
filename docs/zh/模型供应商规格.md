...
- 支持智谱、MiniMax、本地与 OpenAI-compatible 服务；DeepSeek / OpenRouter 等**无专用适配器**，经通用 openai-compatible 适配接入（设 `base_url` + `name` + `api_key`，无独立 provider 别名——直接传 `provider="deepseek"` 会在 factory 抛 `ModelProviderError`）。
...
Provider key、模型路由和账号引用属于运行环境或全局用户配置，不属于项目产物。Windows 默认私有配置目录可使用 `C:\Users\<User>\.asteria\`；项目 Workspace 下的 `.asteria/` 只记录本次 run 的 `ModelCall`、route 名称、模型名、token/成本摘要、timeout/retry/streaming telemetry 和脱敏错误。Workspace evidence、诊断包和最终报告不得包含 API key、本机 route 文件、账号凭证或完整环境变量。

**加载方式（real provider 签字 / 本地验收）**：在操作系统用户级或当前 shell 中设置 `AGENT_MODEL_*`（Windows：「环境变量」→ 用户变量；PowerShell：`$env:AGENT_MODEL_PROVIDER = "minimax"`）。**不要**在 Workspace `.asteria/` 下维护会 dot-source 的 provider 脚本——易与系统配置冲突、含密钥且会过期；验收前用 `python -m asteria_runtime model-check --root . --tier medium --json` 与 `--tier strong` 确认 `call_ok: true`。
...
$env:AGENT_MODEL_CHEAP_PROVIDER = "fake"   # 离线 smoke/演示的显式 opt-in（fake 产出 canned 输出）
```

> 说明：`cheap=fake` 只是 `default_routes` 里带标注的回落，出现在 `model-check` / `doctor` 诊断里；**真实执行路径不默认用它**——未配置 cheap 的运行时由 factory 默认回落 `minimax`。只有显式设了 `*_CHEAP_PROVIDER=fake` 才会真的实例化 fake。

验证阶段的默认策略是 GLM coding route 做 strong/coordinator，MiniMax 做 medium/worker。MiniMax 的规划能力不作为主路线假设，优先用于编码、修复、诊断等明确边界内的执行任务；GLM coding route 优先承接 goal/spec、planning、review、research 和后续 coordinate。本项目当前可先使用 `glm-4.7` 消耗免费 token 额度，额度用完、质量不足或进入最终 release gate 时再切到 `glm-5.1` 复验。未来接入 GPT-5.5 时，应先作为 `strong` route 替换 coordinator，而不是改动 worker runtime。

GLM alias：

- `glm` / `zai` / `z-ai`：默认 `https://open.bigmodel.cn/api/coding/paas/v4`，默认模型 `glm-5.1`，用于智谱 coding 套餐 key，可读取 `ZAI_API_KEY`、`GLM_API_KEY`、`ZHIPU_API_KEY`。如需优先使用 `glm-4.7` 免费额度，应显式设置 `AGENT_MODEL_STRONG_NAME=glm-4.7` 或 `AGENT_MODEL_NAME=glm-4.7`。
- `zhipu` / `bigmodel`：默认 `https://open.bigmodel.cn/api/paas/v4`，默认模型 `glm-5.1`，用于普通 BigModel OpenAI-compatible route，可读取 `ZHIPU_API_KEY`、`BIGMODEL_API_KEY`、`GLM_API_KEY`。

如果实际账号所在平台端点不同，必须显式设置对应 route 的 `*_BASE_URL`。
...
### 6.1.1 Timeout、Retry 与流式返回策略

真实 provider 验收不能把超时只做成外层 subprocess 兜底。默认预算必须从 suite 向下贯穿：

```text
suite budget -> scenario budget -> run/review/recovery budget -> provider call timeout
```

当前验证默认先收紧最坏路径：真实 acceptance 默认每个场景 1 次 run attempt、模型层 1 次 retry、单场景 600 秒；子进程会把 provider call 默认压到 90 秒以内，review/recovery 默认压到 90 秒以内。用户显式配置的 `AGENT_MODEL_<TIER>_TIMEOUT_SECONDS` 和 `AGENT_MODEL_<TIER>_MAX_RETRIES` 优先级高于验证默认值。

产品化目标是尽快把远程 provider route 改为流式优先：

- OpenAI-compatible / GLM / MiniMax route 应支持 streaming chat completion，并记录首 token 时间、chunk 心跳、总生成时间和最后有效 chunk 时间。
- Timeout 语义应从“整段 HTTP read 等死”升级为“连接超时、首 token 超时、chunk idle 超时、总 deadline”四类可解释信号。
- CLI 长任务应把流式进度折叠成低噪声状态，例如当前 phase、模型 route、已收到 token/chunk、最近一次 artifact/evidence 写入时间；默认不打印原始模型输出。
- 非流式仍保留为兼容 fallback，但 validation/release gate 应优先要求真实 strong/medium route 具备流式证据，否则只能进入受限验证。
...
- 能记录 ModelCall。
- 能处理超时和重试。
- `asteria model-check` 失败时写入 `.asteria/model/latest_failure.json`，按 configuration、authentication、rate_limited、timeout、network、server_error、budget、provider_response 等类型分类，并记录到 `.asteria/memory/failures.jsonl`。

### 调用成功与健康度分离

`model-check` 的 `call_ok` 只表示最终获得了有效响应，不代表该路由适合产品主路径。必须同时记录 `call_health`：

- `healthy`：直接完成，未发生 streaming/provider 降级。
- `degraded`：最终成功，但发生 streaming fallback、transport error 或其他可观测降级。
- `failed`：未获得有效响应。
- `skipped`：只检查配置，未发起调用。

真实 provider 灰度不得把 `call_ok=true` 且 `call_health=degraded` 视为健康签字。deadline、首个 chunk、fallback 与总耗时应共同驱动 route 策略。
