...
- 能记录 ModelCall。
- 能处理超时和重试。
- `asteria model-check` 失败时写入 `.agent/model/latest_failure.json`，按 configuration、authentication、rate_limited、timeout、network、server_error、budget、provider_response 等类型分类，并记录到 `.agent/memory/failures.jsonl`。
