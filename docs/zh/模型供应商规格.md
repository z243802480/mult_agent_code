...
灰度阶段的默认策略是 GLM 5.1 做 strong/coordinator，MiniMax 做 medium/worker。MiniMax 的规划能力不作为主路线假设，优先用于编码、修复、诊断等明确边界内的执行任务；GLM 5.1 优先承接 goal/spec、planning、review、research 和后续 coordinate。未来接入 GPT-5.5 时，应先作为 `strong` route 替换 coordinator，而不是改动 worker runtime。

GLM 5.1 alias：

- `glm` / `zai` / `z-ai`：默认 `https://api.z.ai/api/coding/paas/v4`，默认模型 `glm-5.1`，可读取 `ZAI_API_KEY`、`GLM_API_KEY`、`ZHIPU_API_KEY`。
- `zhipu` / `bigmodel`：默认 `https://open.bigmodel.cn/api/paas/v4`，默认模型 `glm-5.1`，可读取 `ZHIPU_API_KEY`、`BIGMODEL_API_KEY`、`GLM_API_KEY`。
