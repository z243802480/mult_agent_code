# Runtime Readiness Matrix and Recovery Pressure Notes

Updated: 2026-05-28

This note records the current implementation boundary for the remaining readiness rollout work.

## Fixed Real-Task Readiness Matrix

The fixed matrix lives at `benchmarks/runtime_readiness_matrix.json` and is consumed by:

- `gate-status` through `runtime_readiness_matrix`
- `readiness-run` through `evidence.runtime_readiness_matrix`
...
The matrix deliberately ignores untracked local `.codex/`, `.claude/`, `readiness_*`, transcript, and probe folders. Those remain local scratch evidence unless promoted into structured runtime artifacts.
...
The readiness matrix keeps this boundary visible by requiring runtime-native progress coverage before a workspace is considered readiness-ready.
...
- `readiness-run` through `evidence.recovery_pressure`
...
- `gate-status --json` fields `runtime_readiness_matrix`, `runtime_progress_metrics`, and `recovery_pressure`.
- `.asteria/readiness_runs/<id>/summary.json` fields under `evidence`.
