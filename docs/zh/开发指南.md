...
$env:AGENT_MODEL_STRONG_API_KEY = "<your glm/z.ai key>"
$env:AGENT_MODEL_STRONG_NAME = "glm-5.1"
...
当前灰度推荐路线是：`strong` 用 GLM 5.1 承担 goal/spec、planning、review、research 和后续 coordinate；`medium` 用 MiniMax 承担 coding、debug、diagnostic 等 worker 执行；`cheap` 先用 fake/local 或低价 OpenAI-compatible route 做分类、摘要和低风险只读任务。后续如果接入 GPT-5.5，可以把 `AGENT_MODEL_STRONG_PROVIDER` 切到对应 OpenAI-compatible route，而不改变 Runtime OS 的 worker 编排结构。
...
$env:AGENT_MODEL_API_KEY = "<your z.ai/glm key>"
