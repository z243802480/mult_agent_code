...
python -m agent_runtime gate-status
python -m agent_runtime version --json
python -m agent_runtime package-check --json
```
...
```powershell
python -m agent_runtime model-check --tier strong
python -m agent_runtime model-check --tier medium
```
...
`real_model_gate.py` 通过后，才进入 core acceptance 或小范围真实任务灰度。
