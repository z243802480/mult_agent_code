...
`status` 看当前 session 和阻塞点，`doctor` 看本地环境与模型路由，`gate-status` 看真实模型 gate、gray suite、core acceptance 的阶段，`version` 看 runtime 版本和 Python 入口。它们都支持或提供 `--json`，后续 TUI / GUI / 自动化灰度门禁必须优先复用这套机器可读控制面输出，避免重新散读 `.agent/` 运行产物。
...
- `gate-status --json` 输出 `release_ready`、`blocking_reason` 和 `gates`，用于判断当前卡在 real model gate、gray suite 还是 core acceptance。
