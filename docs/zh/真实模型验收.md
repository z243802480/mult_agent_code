...
- Runtime OS 场景：`runtime_parallel_readonly`、`runtime_disjoint_writes`、`runtime_worker_failure`、`runtime_merge_gate_block`、`runtime_request_resume`、`runtime_context_package_slice`、`runtime_sandbox_backend_selection`、`runtime_planner_scope_quality`、`runtime_capability_feedback`。
...
`core` / `nightly` release gate 默认要求 Runtime OS 证据：worker invocation/result、runtime profile、context mount、validation result、失败 evidence、candidate isolation、merge gate block、resume recovery、ContextPackage 按 `read_scope` 切片、sandbox backend 选择原因、Planner scope quality 和 capability feedback。只有在兼容旧报告时才使用 `--no-require-runtime-os`。
