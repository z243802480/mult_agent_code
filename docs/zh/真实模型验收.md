...
- Runtime OS 场景：`runtime_parallel_readonly`、`runtime_disjoint_writes`、`runtime_worker_failure`、`runtime_merge_gate_block`、`runtime_request_resume`。
...
`core` / `nightly` release gate 默认要求 Runtime OS 证据：worker invocation/result、runtime profile、context mount、validation result、失败 evidence、candidate isolation、merge gate block 和 resume recovery。只有在兼容旧报告时才使用 `--no-require-runtime-os`。
