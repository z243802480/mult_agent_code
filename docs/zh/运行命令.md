...
- `agent version` / `agent --version` 显示当前 runtime 版本；`agent version --json` 输出 package、version、Python 版本和 executable，供灰度诊断和 bug report 使用。
- `agent package-check` / `agent packaging` 检查本地打包前置条件：pyproject、build backend、console script、版本同步和灰度文档。
- `python scripts/build_package.py --root . --clean` 用当前 Python/pip 构建本地 wheel 到 `dist/`，用于产品灰度安装验证。
- `agent status` 显示当前 Runtime OS 控制面状态：current session、run/task/cost、pending decisions、blockers、latest execution 和推荐下一步。
- `agent doctor` 只读检查本地运行环境：`.agent`、AGENTS、policy/project config、git backend、strong/medium 模型路由和 real-model gate 报告。
- `agent gate-status` 汇总真实模型发布门槛阶段：real_model_gate、gray suite、core acceptance，以及下一步动作。
- `agent version --json`、`agent package-check --json`、`agent status --json`、`agent doctor --json`、`agent gate-status --json` 输出机器可读 JSON；后续 TUI / GUI / 自动化灰度门禁应复用这些控制面数据口径。
...
- `agent model-check --tier strong|medium|cheap` 可按模型层级检查 provider 配置和一次 JSON 健康调用。灰度阶段推荐先检查 `strong=GLM coding route`、`medium=MiniMax`，再进入真实模型 smoke；当前成本优先阶段可先用 `strong=glm-4.7`，发布门槛复验再切到 `glm-5.1`。
- `scripts/real_model_gate.py` 是灰度发布前置 gate：它串联 strong/medium `model-check`、最小真实模型 smoke，并检查 model_calls、worker_results、task_execution_evidence 和 cost_report 证据是否完整。
- `scripts/real_model_acceptance.py --suite gray` 是 gate 之后、core/nightly 之前的灰度候选场景集；summary 中的 `gray_ready` 必须为 true，且 `aggregate.route_evidence` 必须证明 strong/medium route 被实际使用。
...
- `agent acceptance --failed-only` 会读取最新 `acceptance_report.json`，只重跑失败或未闭环的场景。
- 真实验收默认把场景工作区放到 `.agent/acceptance/workspaces/` 下，避免污染仓库根目录。
- `agent acceptance-gate` / `agent release-gate` 会读取最新 `acceptance_report.json` 并作为发布门槛：验收失败未闭环、趋势告警、suite、场景数量、能力覆盖、分层覆盖或 Runtime OS evidence 不足都会返回非零退出码。Runtime OS gate 覆盖并发、worker、merge gate、runtime request resume、ContextPackage 切片、sandbox backend、Planner scope quality、capability feedback 和 debug/review evidence consumption。
- `agent capability-report` 汇总最近 acceptance 趋势、能力覆盖、失败类型、平均修复轮次、模型/工具成本、常见阻塞和推荐动作；同时从 `model_calls.jsonl`、`workers.jsonl`、`worker_results.jsonl`、`validation_results.jsonl` 和 runtime request / merge gate evidence 生成 `.agent/model/capability_profile.json`，按 provider、model、purpose、tier 记录成功率、token 成本、worker 成功率、验证通过率和失败类型，为后续 model routing 提供依据。
- 旧 acceptance 历史如果缺少 `capability` / `tier`，runtime 会根据已知场景名回填，避免能力画像退化为 `unknown`。
- `agent weekly-report` / `agent production-report` 汇总最近 long-run 周期、acceptance、模型画像、风险和下一步动作，并写入 `.agent/reports/weekly_report_<week>.json` 与 Markdown 报告。
- `agent roadmap-update` / `agent prd-update` 从 weekly report、acceptance 和模型画像生成项目级路线图，写入 `.agent/product/project_roadmap.json` 与 `docs/zh/自动路线图.md`。
- 如果 `.agent/model/capability_profile.json` 显示某个 provider/model/purpose route 成功率偏低，`agent long-run-plan` 会优先生成 `model_route_review` 只读动作，提示先检查或切换模型路线，再继续扩大长任务执行预算。
- `agent long-run-plan` 生成的 action 会标注 `responsible_role`，例如 Product、Coder、Debugger、Evaluator；`long-run-report` 和 execution evidence 会保留该字段，方便复盘长任务里是谁负责判断、执行、修复或验收。
- `agent acceptance --promote-failures` 会根据失败 scenario 的 capability 生成定向 `repair_focus`。例如 `multi_file_change` 会强调包布局、入口文件和跨文件 import；`configuration_change` 会强调配置文件是输入输出和过滤逻辑的 source of truth，并只要求单场景复验。
- `agent execute --parallel-disjoint-writes`、`agent run --parallel-disjoint-writes` 和 `agent resume --parallel-disjoint-writes` 会并发执行 `parallel_safety=readonly` 或 `parallel_safety=disjoint_writes` 且 `write_scope` 不冲突的 ready tasks。写任务仍在独立 candidate/worktree 中执行，验证通过后才 promote 回主工作区；冲突写任务留到后续轮次串行处理。
...
- `agent daily-plan` / `agent daily-run` / `agent daily-report` 是早期兼容入口；产品语义已经扩展为长任务运行周期，不限定为自然日。
- `agent long-run-plan` / `agent long-run` / `agent long-run-report` 是推荐入口，支持 `--cycle-id` 和 `--objective`，用于表达“一段可暂停、可恢复、可审计的长任务自动化运行”。
- `agent long-run --execute` 会使用预算守卫控制 action 数、运行时长、模型调用、工具调用和修复次数；失败、超预算或需要人工决策时会硬停，并在报告里写入 `stop_reason`、风险、下一步建议和 `task_execution_evidence.jsonl` 证据路径。
