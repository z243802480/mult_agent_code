...
- `agent acceptance-gate` / `agent release-gate` 会读取最新 `acceptance_report.json` 并作为发布门槛：验收失败未闭环、趋势告警、suite、场景数量、能力覆盖、分层覆盖或 Runtime OS evidence 不足都会返回非零退出码。Runtime OS gate 覆盖并发、worker、merge gate、runtime request resume、ContextPackage 切片、sandbox backend、Planner scope quality 和 capability feedback。
