...
- `agent acceptance-gate` / `agent release-gate` 会读取最新 `acceptance_report.json` 并作为发布门槛：验收失败未闭环、趋势告警、suite、场景数量、能力覆盖或分层覆盖不足都会返回非零退出码。
- `agent capability-report` 汇总最近 acceptance 趋势、能力覆盖、失败类型、平均修复轮次、模型/工具成本、常见阻塞和推荐动作；同时从 `model_calls.jsonl` 生成 `.agent/model/capability_profile.json`，按 provider、model、purpose、tier 记录成功率、token 成本和失败类型，为后续 model routing 提供依据。
