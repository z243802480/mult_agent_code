...
顺序（2026-07-02 P0→P4 重排，用户拍板）：
  ...
  → P4 债务收敛【本批诚实化/债务已落地 → S74-p4-debt-honesty-20260702.md：init 幂等·文档超卖·session_id 去 null·死 MCP 降级 4 项；schema 双向漂移/real_model mypy/Studio 会话 id 全链路/memory·run_command 计分 均撞 DO_NOT_TOUCH → DecisionPoint】+ Track A 外部 Beta 邀请（门槛已满足、等用户拍板）+ re-tag v0.2.0a2（同）
```
