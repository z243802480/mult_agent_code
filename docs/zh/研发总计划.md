...
模型掌舵、程序护栏：标准能力（ContextEnvelope + CapabilityManifest + 三层调度）
由模型选择如何组合；权限/预算/schema/merge 由 Runtime 强制执行。
见 docs/zh/plans/RUNTIME_MULTI_DISPATCH_MODEL.md §8、
docs/zh/plans/ORCHESTRATION_DECISION_POLICY.md（spawn/route 政策真源）。
```
...
S57：docs/zh/reports/S57-harness-accept-b6-regression-signoff-20260607.md
S56：docs/zh/reports/S56-beta-task-pack-pulse-signoff-20260607.md
三线：docs/zh/plans/TRIPLE_TRACK_MAINT_PLAN.md
脉搏：scripts/triple_track_pulse.py
```
