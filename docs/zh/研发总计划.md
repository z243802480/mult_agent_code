...
### 3.2 In Progress（设计已有，实现未完成）

- `user_progress` 全路径 `transcript_kind` + `ui_intent` + `display_level=main`
- CLI status 与 Studio Thread **叙事一致**
- Goal/Plan/Ask 用户面；maintainer 命令隐藏
- Studio 删 **冒充完成**（`localGeneralAnswer` 等）
- CapabilityManifest 注入模型入口
- Capability catalog 与真实选择对齐

### 3.3 Designed-not-built（文档设计，Phase 3+）

- North Star `north_star.json` + 跨 run milestone
- 真实 disjoint write workers + sandbox 放量
...
Phase 1 进度协议与 S1b MERGE 已完成；Phase 2 Studio 对齐（S5–S6）已完成。当前 **ACTIVE_SLICE：S7**（MVP 终点：run 级 benchmark 契约已绿，待 real provider 签字，见 `docs/zh/reports/S7-mvp-signoff-pending.md`）。签字后进入 **Phase 3 / S8**。详见 [`AGENTS.md`](../../AGENTS.md) 与 [`benchmarks/vibe_slices.json`](../../benchmarks/vibe_slices.json)。
