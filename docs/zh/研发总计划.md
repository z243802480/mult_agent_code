...
| 1.2.18 | 2026-07-09 | **ADR-0024 #1 快照按 task 刷新（B）+ 自主环 A 真栈验证（发现 goal-replan 收敛 bug）**。**B（已提交 `a00185c`）**：`ContextLoader.workspace_files()` 抽公开独立重扫，`ExecuteCommand._execute_task` 每 task 起手用 `context.root` 重扫覆盖 `runtime_context["workspace_files"]`（该 dict 已 per-task 不污染共享 context·best-effort `except OSError`）→ 多 task run 后续 task 看得见前一 task 刚写的文件，不再盲重建（用户观察"重复写"根因最后一块）。+1 单测·context/execute/run/replan 64 绿·mypy/ruff 净。**A（真栈验证·throwaway workspace·三 flag `auto_repair`/`auto_replan`/`auto_replan_goal` 全 armed·glm-5+minimax）**：① `ring_val_a` happy-path——模型首过写对，独立 pytest **4 passed**，armed 三环不破坏真栈 happy-path（比首次点火多的数据点：点火时三环 OFF）；② `ring_val_b` 强制首验失败——预置确定性 buggy 基线（空串返 0 而非 raise），**goal-level replan 环（ADR-0017）真触发**：task-0001 blocked→自动取代 0002→0003→0004，前三 discarded（正是 1.2.16 finalizer 修的 discarded 终态路径）。**⚠️ A 抓到真收敛 bug（fixture 测漏）**：(a) 被取代新任务 `metadata={}` **空**，无 parent/lineage 深度→ReplanCommand 无从知 0004 是 0001 三代后裔→`max_replans_per_task=2` 永不跨链累积→`decisions.jsonl=0` 从未升 DecisionPoint→环无限自我取代，仅被 run 级 budget 兜住；(b) `validation_results` 显示后续 pytest **已 passed**、产物独立 **4/4 passed** 时环仍 block→再取代，验证已过未判 done。**结论=三环真栈会 fire 但 goal-replan 有收敛/终止缺陷（真栈才暴露）→这是三环必须默认关、翻默认前必先修收敛的硬证据**（补上 [[flywheel-first-ignition-proven]] 的"三环武装未触发"缺口并翻出 bug）。修复方向（未做·触 run_command/ReplanCommand·解冻内）：给取代任务写 lineage(parent+depth)按链深累积 cap；任务级"验证已过即 done"要先于 goal-replan 取代。详见记忆 `ring-realstack-validation-A` |
