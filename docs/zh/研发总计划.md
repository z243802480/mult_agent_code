...
- **近期第一批（小时~天级，无战略依赖，建议先做）**：加 LICENSE + pyproject `[project.license]` 元数据（先决 A）；假默认档改 fail-loud（静默罐头→显式报错/拒绝）；版本/wheel/ACTIVE_SLICE 对齐 + package-check 真构建 + docs 去掉不存在的 S69 引用。
