...
顺序（2026-07-02 P0→P4 重排，用户拍板）：
  ...
  → P1 信任修复（把说出去的话兑现或收回；含原 D5 compact 文案/final 误标）
  → P2 核心闭环断点（skill/MCP 回灌、/run review、修复预算、chat 历史）
  → P3 桌面赌注（Stop/中断、会话搜索、运行态流式、费用显示、Inspector raw evidence）
  → P4 债务收敛 + Track A 外部 Beta 邀请（P0+P1 之后）+ re-tag v0.2.0a2
```
