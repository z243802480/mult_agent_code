...
function ItemIcon({ state }: { state: PlanItemState }) {
  ...
  if (state === "in_progress") return <Loader2 size={13} className="planItemIcon active spinning" />;
...
export function PlanChecklist({ plan, defaultOpen = true }: { plan: PlanModel; defaultOpen?: boolean }) {
