...
check("reviewed_auto -> balanced", () => assert.equal(mapPermissionLevel("reviewed_auto"), "balanced"));
check("auto -> auto", () => assert.equal(mapPermissionLevel("auto"), "auto"));
check("unknown -> balanced (product default)", () => assert.equal(mapPermissionLevel("nonsense"), "balanced"));
...
check("splices --permission-level right after the run token", () => {
  ...
  assert.deepEqual(withPermissionLevel(cmd, "auto"), [
    "python", "-m", "asteria_runtime", "run", "--permission-level", "auto", "--root", "ws", "--no-research", "goal",
  ]);
});

check("works for run --continue-session", () => {
  const cmd = ["python", "-m", "asteria_runtime", "run", "--continue-session", "--root", "ws", "goal"];
  assert.deepEqual(withPermissionLevel(cmd, "ask"), [
    "python", "-m", "asteria_runtime", "run", "--permission-level", "ask", "--continue-session", "--root", "ws", "goal",
  ]);
});
...
check("never double-adds the flag", () => {
  const cmd = ["python", "-m", "asteria_runtime", "run", "--permission-level", "ask", "--root", "ws"];
