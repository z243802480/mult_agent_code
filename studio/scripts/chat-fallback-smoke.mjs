...
try {
  ...
  assert(
    /Goal understanding|Recommended plan|Suggested sequence|Quick answer|I could not reach/i.test(
      answerText,
    ),
    "fallback should be user-facing, natural, and useful",
  );
  assert(
    !/Temporary local fallback|Request type:|intent|route|model route|run was started|files were changed/i.test(
      answerText,
    ),
    "fallback should not expose backend metadata",
  );
  assert(
    !/Zhanqiao|Laoshan|May Fourth|Beer Museum|shadowing/i.test(answerText),
    "fallback should not contain domain-specific template content",
  );
  assert(
    !/Context refs:|Current session:|Next actions:|Evidence Explorer|Inspector|run-[0-9]/i.test(
      answerText,
    ),
    "fallback should not expose CLI/runtime context noise",
  );
...
async function fetchEventsUntil(sessionId, predicate) {
  ...
  throw new Error(
    `Timed out waiting for fallback answer. events=${(latest.events || []).length} stdout=${stdout} stderr=${stderr}`,
  );
}
