...
await fs.writeFile(path.join(workspace, ".asteria", "project.json"), JSON.stringify({
  ...
  frameworks: [],
  package_managers: [],
  commands: { install: null, run: null, test: null, lint: null, typecheck: null, build: null, format: null },
  important_paths: [],
  protected_paths: [".env", "secrets/", ".git/"],
  root_guidance_path: "AGENTS.md",
  default_policy_path: ".asteria/policies.json",
}, null, 2));
await fs.writeFile(path.join(workspace, ".asteria", "policies.json"), JSON.stringify({
  schema_version: "0.1.0",
  ...
  context: { compaction_threshold: 0.75, hard_stop_threshold: 0.9, phase_boundary_compaction: false, handoff_compaction: false },
  permissions: {
    allow_network: false,
    allow_shell: false,
  ...
  protected_paths: [".env", "secrets/", ".git/"],
  hooks: { enabled: false, plugins_enabled: false, allowed_hook_names: [], redacted_data_keys: [], handler_timeout_ms: 1000 },
  promotion: { manual_approval_default: true, release_blocking_statuses: [], max_pending_release_promotions: 0, max_blocked_release_promotions: 0 },
  model_routing: {},
  commands: {},
}, null, 2));
...
try {
  ...
  assert(user?.content_delta === message, `expected Chinese user message to round-trip, got ${user?.content_delta}`);

  const modelStart = events.find((event) => event.type === "model_start" && event.phase === "chat" && String(event.event_id || "").startsWith("evt-model-"));
  ...
  const delta = events.find((event) => event.type === "model_delta" && event.phase === "chat" && event.parent_event_id === modelStart.event_id);
  ...
  assert(!/Context refs:|Current session:|Next actions:/i.test(answerText), "final answer should not expose CLI context noise");
  assert(!/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/.test(answerText), "final answer should not contain control characters");
