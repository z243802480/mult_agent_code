            ...
            actions.append(
                {
                    ...
                    "command": "python -m asteria_runtime /decide --list-pending --root .",
            ...
            actions.append(
                {
                    ...
                    "command": "python -m asteria_runtime /acceptance --root . --failed-only",
            ...
            actions.append(
                {
                    ...
                    "command": "python -m asteria_runtime /execute --root . --max-tasks 1",
            ...
            actions.append(
                {
                    ...
                    "command": "python -m asteria_runtime /debug --root . --max-repairs 1",
...
class DailyRunCommand:
    ...
    def _run_action(self, action: dict[str, Any], guard: DailyBudgetGuard) -> dict[str, Any]:
        ...
        command = [sys.executable, "-m", "asteria_runtime", *self._command_args(action)]
        env = os.environ.copy()
    ...
    def _next_actions(self, results: list[dict[str, Any]]) -> list[str]:
        ...
        if any(item["status"] == "fail" for item in results):
            return [
                ...
                "Run `asteria /replan` or `asteria /debug` only after the failure type is understood.",
            ]
