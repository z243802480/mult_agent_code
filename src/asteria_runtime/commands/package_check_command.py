...
class PackageCheckResult:
    ...
    def _next_actions(self, failed: list[str]) -> list[str]:
        actions: list[str] = []
        if failed:
            actions.append("Fix package metadata and version checks before building a validation package.")
        if "validation_route_template" in failed:
            actions.append("Add `templates/model.routes.validation.example.ps1` before validation run.")
        if "validation_command_modules" in failed:
            actions.append("Package real-model gate and acceptance modules before validation run.")
        if "hook_plugin_control_surface" in failed:
            actions.append("Package hook/plugin schemas, command modules, defaults, and docs.")
        if "validation_runbook" in failed:
            actions.append("Add `docs/zh/验证试运行手册.md` before validation run.")
        if not self.artifacts:
            actions.append("Build a local wheel with `python -m build` when the build dependency is available.")
        actions.append("Follow `docs/zh/验证试运行手册.md` for install, route config, self-check, rollback, and failure collection.")
        actions.append("Configure model routes from `templates/model.routes.validation.example.ps1` in the target shell.")
        actions.append("Run `asteria version --json` from the installed package before validation run.")
        return actions

    def _runbook(self) -> dict[str, object]:
        return {
            "path": "docs/zh/验证试运行手册.md",
            "required_sections": [
                "install",
                "route_config",
                "self_check",
...
class PackageCheckCommand:
    ...
    def _validation_command_modules_check(self) -> PackageCheck:
        required = [
    ...
    def _route_template_check(self) -> PackageCheck:
        path = self.root / "templates" / "model.routes.validation.example.ps1"
        return PackageCheck(
            "validation_route_template",
            path.exists(),
            str(path.relative_to(self.root)) if path.exists() else "missing validation route template",
        )
    ...
    def _runbook_docs_check(self) -> PackageCheck:
        ...
        return PackageCheck(
            "validation_docs",
            not missing,
            "validation run docs present" if not missing else "missing: " + ", ".join(missing),
        )

    def _validation_runbook_check(self) -> PackageCheck:
        path = self.root / "docs" / "zh" / "验证试运行手册.md"
        return PackageCheck(
            "validation_runbook",
            path.exists(),
            str(path.relative_to(self.root)) if path.exists() else "missing validation runbook",
        )
