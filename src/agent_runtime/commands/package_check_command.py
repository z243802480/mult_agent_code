...
class PackageCheckCommand:
    ...
    def _pyproject_check(self, pyproject: dict) -> PackageCheck:
        ...
        ok = name == "agent-runtime" and bool(version)
        return PackageCheck(
            "pyproject",
            ok,
    ...
    def _console_script_check(self, pyproject: dict) -> PackageCheck:
        ...
        target = scripts.get("agent") if isinstance(scripts, dict) else None
        ok = target == "agent_runtime.cli:main"
        return PackageCheck(
            "console_script",
            ok,
            f"agent={target or 'missing'}",
        )
