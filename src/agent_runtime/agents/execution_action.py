...
def _repair_tool_args(tool_name: str, args: dict, task: dict) -> dict:
    ...
    if tool_name == "run_command" and isinstance(repaired.get("command"), str):
        repaired["command"] = _normalize_verification_command(repaired["command"], task)
    return {key: value for key, value in repaired.items() if value not in (None, "")}


def _normalize_verification_command(command: str, task: dict) -> str:
        ...
        if echo_match:
            ...
            files.append(
                (
                    ...
                    (double_quoted if double_quoted is not None else single_quoted if single_quoted is not None else unquoted).strip(),
                )
            )
            continue
