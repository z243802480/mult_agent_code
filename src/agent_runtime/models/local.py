...
def local_settings_from_env(provider: str) -> OpenAICompatibleSettings:
    defaults = LOCAL_PROVIDER_DEFAULTS[provider]
    model_name = os.getenv("AGENT_MODEL_NAME") or defaults.model_name
    if not model_name:
        raise OpenAICompatibleProviderError(
            f"AGENT_MODEL_NAME is required for local provider '{provider}'."
        )
    return OpenAICompatibleSettings(
        api_key=os.getenv("AGENT_MODEL_API_KEY") or defaults.api_key,
        base_url=(os.getenv("AGENT_MODEL_BASE_URL") or defaults.base_url).rstrip("/"),
        model_name=model_name,
        provider=provider,
        timeout_seconds=int(os.getenv("AGENT_MODEL_TIMEOUT_SECONDS", "180")),
