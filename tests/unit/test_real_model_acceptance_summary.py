...
def test_full_suite_writes_canonical_acceptance_summary(tmp_path: Path) -> None:
    args = argparse.Namespace(
        suite="validation",
        scenario=[],
        summary_json=tmp_path / "custom_validation.json",
    )

    paths = summary_output_paths(args, tmp_path)

    assert paths == [
        (tmp_path / "custom_validation.json").resolve(),
        tmp_path / ".asteria" / "verification" / "real_model_acceptance_validation.json",
    ]
...
def test_scenario_subset_does_not_overwrite_canonical_summary(tmp_path: Path) -> None:
    args = argparse.Namespace(
        ...
        summary_json=tmp_path / "validation_refactor.json",
    )

    paths = summary_output_paths(args, tmp_path)

    assert paths == [(tmp_path / "validation_refactor.json").resolve()]
