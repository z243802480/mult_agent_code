...
def test_full_suite_writes_canonical_acceptance_summary(tmp_path: Path) -> None:
    args = argparse.Namespace(
        suite="gray",
        scenario=[],
        summary_json=tmp_path / "custom_gray.json",
    )

    paths = summary_output_paths(args, tmp_path)

    assert paths == [
        (tmp_path / "custom_gray.json").resolve(),
        tmp_path / ".asteria" / "verification" / "real_model_acceptance_gray.json",
    ]
...
def test_scenario_subset_does_not_overwrite_canonical_summary(tmp_path: Path) -> None:
    args = argparse.Namespace(
        suite="gray",
        scenario=["gray_refactor"],
        summary_json=tmp_path / "gray_refactor.json",
    )

    paths = summary_output_paths(args, tmp_path)

    assert paths == [(tmp_path / "gray_refactor.json").resolve()]
