...
def test_sandbox_backend_selector_allows_untracked_process_artifacts(tmp_path: Path) -> None:
    (tmp_path / "tool.py").write_text("VALUE = 1\n", encoding="utf-8")
    (tmp_path / "real_model_smoke_transcript.json").write_text("{}", encoding="utf-8")
    try:
        subprocess.run(["git", "init"], cwd=tmp_path, check=True, capture_output=True, text=True)
        subprocess.run(
            ["git", "config", "user.email", "agent@example.test"],
