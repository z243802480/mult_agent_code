...
def test_execution_coordinator_runs_parallel_batch_with_preallocated_worker_ids(
    ...
    def execute_task(task: dict, worker_id: str | None, result_id: str | None) -> str:
        calls.append((task["task_id"], worker_id, result_id))
        return task["task_id"]
    ...
    results = coordinator.execute_selection(
        ...
        allocate_worker_ids=lambda count: [f"worker-{index:04d}" for index in range(1, count + 1)],
        allocate_worker_result_ids=lambda count: [
            f"worker-result-{index:04d}" for index in range(1, count + 1)
        ],
    )
